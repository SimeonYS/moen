...
class MoenPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('moen.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `moen`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM moen WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO moen (date, title, link, content)"
